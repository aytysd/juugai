  ...
  createEvalFunc(measuredDist, cordinalDirection) {
    ...
    switch (cordinalDirection) {
      ...
      case "East":
        range = (theta) => {
          return ( 
            (7*Math.PI/4 <= theta && theta < 2*Math.PI) ||
...
//     }
    ...
    return (vec) => {
      ...
      if(range(polar.theta)) {
        if( 0 <= polar.r && polar.r < measuredDist ) return this.evalWithinD(vecFromNode, measuredDist);
        else return this.evalBeyondD(vecFromNode, measuredDist);
      } else {
        return 0; 
      }
    }
